...
class HelloTriangleApplication
{
  ...
  private:
	RTE::Platform::RTEWindow *_window;
	...
	void initWindow()
	{
		...
		_window = wm->OpenWindow(WIDTH, HEIGHT, "vulkan HELLO THERE");
	}
	...
	bool isDeviceSuitable(VkPhysicalDevice device)
	{
		...
		std::cout << deviceProperties.limits.maxMemoryAllocationCount << std::endl;
	...
	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR> &availableFormats)
	{
		...
		return availableFormats[0]; //Possible to go through the formats and choose the best, but it also works with the first one available.
	}

	VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR> availablePresentModes)
	{
		VkPresentModeKHR bestMode = VK_PRESENT_MODE_FIFO_KHR; //This mode is guaranteed?
	...
	SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device)
	{
		...
		vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, *_window->surface, &details.capabilities);
		uint32_t formatCount;
		vkGetPhysicalDeviceSurfaceFormatsKHR(device, *_window->surface, &formatCount, nullptr);

		if (formatCount != 0)
		{
			details.formats.resize(formatCount);
			vkGetPhysicalDeviceSurfaceFormatsKHR(device, *_window->surface, &formatCount, details.formats.data());
		}

		uint32_t presentModeCount;
		vkGetPhysicalDeviceSurfacePresentModesKHR(device, *_window->surface, &presentModeCount, nullptr);

		if (presentModeCount != 0)
		{
			details.presentModes.resize(presentModeCount);
			vkGetPhysicalDeviceSurfacePresentModesKHR(device, *_window->surface, &presentModeCount, details.presentModes.data());
		}
		return details;
	}
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device)
	{
		...
		for (const auto &queueFamily : queueFamilies)
		{
			...
			vkGetPhysicalDeviceSurfaceSupportKHR(device, i, *_window->surface, &presentSupport);
	...
	void mainLoop()
	{
		...
		while (!RTE::Platform::ShouldClose(_window))
		{
	...
	void cleanup()
	{
		...
		if (enableValidationLayers)
		{
			DestroyDebugUtilsMessengerEXT(*_window->instance, callback, nullptr);
		}

		vkDestroySurfaceKHR(*_window->instance, *_window->surface, nullptr);
		vkDestroyInstance(*_window->instance, nullptr);

		RTE::Platform::DestroyWindow(_window);
